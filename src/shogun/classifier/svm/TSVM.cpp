...
CTSVM::CTSVM()
: CLinearMachine(), C1(1), C2(1), epsilon(1e-6), use_bias(true)
{
}

CTSVM::CTSVM(
	float64_t C, CDotFeatures* traindat, CLabels* trainlab)
: CLinearMachine(), C1(C), C2(C), epsilon(1e-6), use_bias(true)
{
...
bool CTSVM::train_machine(CFeatures* data)
{
	ASSERT(m_labels);
	...
	Data.l=num_vec-Data.u;
	Data.n=num_feat+1;
	Data.nz=num_feat+1;
	Data.features=features;
	Data.C = SG_MALLOC(float64_t, Data.m);
